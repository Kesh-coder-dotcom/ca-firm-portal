...
def get_new_messages(after_timestamp):
    rows = (
        get_client()
        .table("messages")
        .select("*")
...
# ══════════════════════════════════════════════════════════════════
# 7. DATABASE - FILE ATTACHMENTS (NEW)
# ══════════════════════════════════════════════════════════════════

MAX_FILE_BYTES = 1 * 1024 * 1024


def upload_file_attachment(task_id, uploaded_file):
    actor = current_user()
    if not actor:
        ...
        for att in attachments:
            ...
            with col_a:
                info = (
                    att["file_name"]
                    + " | " + str(size_kb) + " KB"
                    + " | Uploaded by " + att["uploader_name"]
                    + " on " + ts
                )
                st.markdown(info)
                if att.get("file_data"):
                    try:
                        decoded = base64.b64decode(att["file_data"])
                        st.download_button(
                            label="Download " + att["file_name"],
                            data=decoded,
            ...
            with col_b:
                can_delete = (
                    current_user().get("role") == "master"
                    or current_user().get("id") == att["uploader_id"]
                )
                if can_delete:
                    if st.button("Remove", key="rm_att_" + att["id"]):
    ...
    with st.form(key="upload_" + task_id):
        ...
        if st.form_submit_button("Upload File"):
            if uploaded is None:
                ...
                ok, msg = upload_file_attachment(task_id, uploaded)
                if ok:
                    st.success(msg)
                    st.rerun()
                else:
...
def page_new_task():
    ...
    with st.form("new_task_form", clear_on_submit=True):
        ...
        description = st.text_area(
            ...
            height=160,
            placeholder=(
                "Client name, documents required,"
                " special instructions, deadlines..."
            ),
        )
        if st.form_submit_button(
            ...
            if not title.strip():
                ...
                st.error(
                    "At least one HOD and one Staff member"
                    " must exist before creating a task."
                )
            else:
                result = create_task({
                    "title": title.strip(),
...
def page_messaging():
    ...
    st.caption(
        "Real-time group chat — visible to all team members."
    ...
    if not st.session_state["all_messages"]:
        st.session_state["all_messages"] = get_messages(limit=100)
        if st.session_state["all_messages"]:
            st.session_state["last_msg_time"] = (
                st.session_state["all_messages"][-1]["created_at"]
            )
    ...
    if col_r.button("Refresh Now"):
        if st.session_state.get("last_msg_time"):
            new_msgs = get_new_messages(
                st.session_state["last_msg_time"]
            )
            if new_msgs:
                st.session_state["all_messages"].extend(new_msgs)
                st.session_state["last_msg_time"] = (
                    new_msgs[-1]["created_at"]
                )
        else:
            st.session_state["all_messages"] = get_messages(limit=100)
        st.rerun()
    col_info.caption(
        "Click Refresh Now to load new messages from others."
    )
...
def page_users():
    ...
    with tab_list:
        ...
        if rf == "All":
            ...
            filtered = [u for u in users if u["role"] == rf]
        if not filtered:
...
def render_sidebar(user):
    with st.sidebar:
        ...
        st.markdown(
            ...
            "<span style='color:" + rc + ";text-transform:capitalize'>"
            + role + "</span></div>",
            unsafe_allow_html=True,
        )
        st.divider()
