...
def register_team_master(
    ...
    """
    Creates the team and the master user in one flow.
    ...
    try:
        # Create auth user - Supabase sends verification email
        auth_resp = get_client().auth.sign_up(
            {
                "email": email.strip(),
        ...
        return True, (
            "Team created! Please check your email ("
            + email.strip()
    ...
    with col:
        ...
        with tab_register:
            ...
            st.markdown(
                ...
                "Verify it, then come back and log in."
            )
...
def render_sidebar(user):
    with st.sidebar:
        ...
        st.markdown(
            ...
            "TEAM</span><br>"
            "<b style='font-size:1rem;color:#0ea5e9'>"
            + team_name + "</b>"
            "</div>",
            unsafe_allow_html=True,
        )
