...
def _build_chat_html(messages, my_id):
    """Build the chat bubble HTML string from a list of messages."""
    chat_html = (
        "<div style='"
        "max-height:360px;"
        "overflow-y:auto;"
        ...
        "gap:8px;"
        "margin-bottom:12px;"
        "box-shadow:0 0 20px #bf00ff22,inset 0 0 20px #00000066'>"
    )
    if not messages:
        chat_html += (
            "<p style='"
        ...
        for msg in messages:
            ...
            rc = ROLE_COLORS.get(msg.get("author_role", ""), "#888")
            ...
            if is_me:
                ...
                bubble = (
                    ...
                    "<div style='"
                    "max-width:75%;"
                    "background:linear-gradient(135deg,#0d0820,#1a0a35);"
...
def _refresh_chat_placeholder(placeholder, messages, my_id):
    """Re-render only the chat section inside an st.empty() placeholder."""
    placeholder.markdown(
        _build_chat_html(messages, my_id),
...
def render_task_discussion(task):
    ...
    st.caption(
        "Visible only to Master, assigned HOD and Staff. "
        "Auto-refreshes every 5 seconds."
    )

    # ── Real-time: auto refresh every 5 seconds ───────────────
    # Each task gets its own refresh key in session state
    refresh_key = "last_refresh_" + task_id
    msgs_key = "msgs_" + task_id

    # Initialize message cache for this task
    if msgs_key not in st.session_state:
        st.session_state[msgs_key] = get_task_messages(task_id)

    # Poll for new messages since last known message
    current_msgs = st.session_state[msgs_key]
    if current_msgs:
        last_ts = current_msgs[-1]["created_at"]
        new_msgs = (
            get_client()
            .table("task_comments")
            .select("*")
        ...
        if new_msgs:
            ...
            st.session_state[msgs_key].extend(new_msgs)

    messages = st.session_state[msgs_key]

    # ── Chat window ───────────────────────────────────────────
    chat_placeholder = st.empty()
    # Render chat inside placeholder so it can be updated without full page reload
    _refresh_chat_placeholder(chat_placeholder, messages, my_id)

    # ── Send message ──────────────────────────────────────────
    with st.form(
        key="msg_" + task_id, clear_on_submit=True
    ):
        ...
        if bc.form_submit_button(
            "Send", use_container_width=True
        ):
            if new_msg.strip():
                send_task_message(task_id, new_msg.strip())
                # Reload only messages and re-render chat section
                updated = get_task_messages(task_id)
                st.session_state[msgs_key] = updated
                # Rebuild chat HTML in-place without full page rerun
                _refresh_chat_placeholder(
                    chat_placeholder, updated, my_id
                )

    # ── JS-based auto-poll: refreshes only the message section ──
    # Uses a hidden iframe counter; Streamlit checks session state
    # and re-renders only the chat placeholder every 5 seconds.
    poll_key = "poll_count_" + task_id
    if poll_key not in st.session_state:
        st.session_state[poll_key] = 0

    # Inject JS that increments a counter via query param after 5s
    # Streamlit's st_autorefresh equivalent using component HTML
    st.components.v1.html(
        """
        <script>
        (function() {
            // Only register one interval per task chat
            var key = 'chat_poll_registered_""" + task_id + """';
            if (window[key]) return;
            window[key] = true;
            setInterval(function() {
                // Trigger Streamlit rerun by clicking hidden button
                var btn = window.parent.document.querySelector(
                    '[data-testid="stButton"] button[kind="secondary"][id="poll_btn_""" + task_id + """"]'
                );
                if (btn) btn.click();
            }, 5000);
        })();
    ...
    # Hidden button that JS clicks to trigger a targeted poll
    col_poll, col_r, col_info = st.columns([1, 1, 3])
    with col_poll:
        # Invisible poll trigger — JS clicks this every 5 s
        poll_clicked = st.button(
            "↻",
            key="poll_btn_" + task_id,
            help="Auto-polling for new messages",
        )
        if poll_clicked:
            new_data = get_task_messages(task_id)
            if len(new_data) != len(st.session_state[msgs_key]):
                st.session_state[msgs_key] = new_data
                _refresh_chat_placeholder(
                    chat_placeholder, new_data, my_id
                )
    with col_r:
        if st.button(
            "Refresh Chat", key="ref_" + task_id
        ):
            st.session_state[msgs_key] = get_task_messages(
                task_id
            )
            _refresh_chat_placeholder(
                chat_placeholder,
    ...
    col_info.caption(
        "Auto-polls every 5 seconds (chat only)."
    )
    ...
    with st.form(
        key="upl_" + task_id, clear_on_submit=True
    ):
        st.markdown(
            "<span style='"
            "color:#39ff14;"
